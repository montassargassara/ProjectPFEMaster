...
@Component({
      ...
      <section class="container body">
        ...
        <aside class="sidebar">
          <div class="contact-card">
            <div class="agency">
              <div class="agency-avatar">{{ agencyInitials() }}</div>
              <div>
                <span class="muted">Agence partenaire</span>
                <strong>{{ property.agencyName || 'Agence Maison3D' }}</strong>
              </div>
            </div>
            <button class="btn-primary" type="button" (click)="onInterested()">
              Intéressé par ce bien
            </button>
            <p class="login-hint" *ngIf="!isLoggedIn()">
              Connectez-vous pour envoyer votre intérêt à l'agence.
            </p>
            <p class="reassure">
              Réponse rapide · Annonce vérifiée · Sans frais cachés
            </p>
          </div>
        </aside>
      </section>
  ...
  styles: [
    `
      ...
      .agency .muted { font-size: 12px; color: #64748b; display: block; }
      .agency strong { font-size: 14px; color: #0f172a; }
      ...
      .btn-primary:hover {
        ...
        box-shadow: 0 8px 20px -4px rgba(11, 107, 203, 0.45);
      }
      .btn-secondary {
        background: #fff;
        border: 1px solid #e2e8f0;
      ...
      .btn-secondary:hover { border-color: #0b6bcb; color: #0b6bcb; }
      .reassure {
        margin: 12px 0 0;
        font-size: 12px;
        color: #64748b;
...
export class PublicPropertyDetailComponent implements OnInit, OnDestroy {
  ...
  agencyInitials(): string {
    const name = this.property?.agencyName || 'M3D';
    return name
      .split(/\s+/)
      .map((s) => s.charAt(0))
      .join('')
